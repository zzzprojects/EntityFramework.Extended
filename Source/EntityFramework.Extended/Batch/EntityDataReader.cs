...
namespace EntityFramework.Batch
{
    public class EntityDataReader<TEntity> : IDataReader where TEntity : class
    {
        ...
        public EntityDataReader(IEnumerable<TEntity> entities, EntityMap entityMap)
        {
            ...
            {
                _columnNameIndex[_entityMap.PropertyMaps[idx].ColumnName] = i;
                _propInfos[i] = tEntity.GetProperty(_entityMap.PropertyMaps[idx].PropertyName);
            }
        ...
        public bool Read()
        {
            ...
            if (_current != null) for (int i=0; i<_propInfos.Length; i++) _values[i] = _propInfos[i].GetValue(_current);
            return isNext;
        }
    }
}
